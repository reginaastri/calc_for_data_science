...
```
    + The *gradient* of a function is a vector of its partial deriviatives. It is usually symbolized by a nabla (upside down triangle,) as illustrated in the figure.
    ```{r fig.show='hold', fig.align='center', echo=FALSE}
...
3. Gradient ascent
    + Ascend Maunga Whau by gradient ascent.
    ```{r fig.show='hold', fig.align='center', echo=FALSE}
bg <- par("bg")
par(bg="lightblue")
...
plunk <- function(v, col="yellow"){
  i <- ceiling(10e-10+v[1]/10)
  j <- ceiling(10e-10+v[2]/10)
  temp <- trans3d(x[i], y[j], z[i,j], res)
...
# Choose a starting point and watch the hiker climb.
# v <- c(520, 550) # might work as a starting point.
# v <- c(800, 400) # also
# v <- c(349.9812, 592.6295)
```


    ```{r fig.show='hold', fig.align='center', echo=FALSE, include=FALSE}
# Decided to chuck this in favor of gradient ascent.
mai <- par("mai")
par(mai=adj_margins())
